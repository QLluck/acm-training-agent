...
export function CoachDashboard() {
  ...
  const overview = teamOverview(students, results);
  const focus = recommendTeamFocus(students, results);
  const watchlist = getWatchlist(students);
  const [level, setLevel] = useState("全部队员");
  const [notice, setNotice] = useState("");
  const [topic, setTopic] = useState("图上状态设计");
  const [extraStudent, setExtraStudent] = useState("zhou");
  const [difficulty, setDifficulty] = useState("-100");
  const team = students.filter(
    (s) => level === "全部队员" || s.level === level,
  );
  const accepted = state.decisions.some((d) => d.kind === "accept");

  function exportPlan() {
    const content = [
      "ACM Training Agent · 模拟周训练计划",
      `目标难度调整：${state.difficulty >= 0 ? "+" : ""}${state.difficulty}`,
      ...weeklyPlan.map(
        (p) => `${p.day}：${p.title}（${p.minutes} 分钟）\n原因：${p.reason}`,
      ),
      state.topic ? `全队追加专题：${state.topic}` : "",
  ...
  return (
    <>
      <PageHeading
        eyebrow="COACH WORKSPACE / TEAM INSIGHTS"
        title="看清全队状态，安排下一步训练。"
        description="ACM 集训队 · 2026 秋季训练周期 · 第 4 周（模拟）"
        action={
      ...
      <div className="coach-analysis-grid">
        <section className="card heatmap-card">
          <SectionTitle
            title="全队能力热力图"
            subtitle="每行是一名队员，每列是一项能力 · 点击姓名查看个人画像"
            aside={
              <label className="filter-select">
                <span className="sr-only">筛选队员层级</span>
                <select
                  value={level}
                  onChange={(e) => setLevel(e.target.value)}
                >
                  {["全部队员", "新人组", "进阶组", "主力组"].map((v) => (
          ...
          <div className="heatmap-scroll">
            <table className="heatmap">
              <caption className="sr-only">
                模拟队员的六维能力值，0 到 100。颜色越深表示能力值越高。
              </caption>
              <thead>
                <tr>
                  <th scope="col">队员 / Rating</th>
          ...
          <div className="heatmap-legend">
            <span>当前展示 {team.length} / 12 名队员</span>
            <div>
          ...
          <div>
            ...
            <button
              ...
              onClick={() => {
                demoActions.decide(
                  "topic",
                  `周五为全队追加「${topic}」专题 45 分钟`,
                  { topic },
                );
                setNotice(`全队专题「${topic}」已加入周五安排。`);
              }}
            >
              给全队增加专题
            </button>
